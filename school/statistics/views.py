...
from .forms import SelectStatisticForm, TeacherWBOWROStatisticForm, OrderedStudentsStatisticFrom, \
    TeacherDisciplinesStatisticForm
    ...
    if statistic_id == 0:
        ...
        if ordered_students_statistic_form.validate_on_submit():
            ...
            subquery = db.session.query(User.id, User.realname.label("users_realname"), Group.name.label("groups_name"),
                                        ...
                                        Semester.year.label("semester_start_year")).join(Group.students) \
                .join(Enrollment).group_by(Enrollment.student_id).filter(Enrollment.grade > 0).join(
                degrees_period_students).join(DegreePeriod). \
        ...
        return render_template("statistics/generate_statistic.html", statistic_id=statistic_id,
                               form=ordered_students_statistic_form)

    elif statistic_id == 1:
        teacher_wbowro_statistic_form = TeacherWBOWROStatisticForm()
        if teacher_wbowro_statistic_form.validate_on_submit():
            subquery = db.session.query(User.realname.label("users_realname"), Course.name.label("courses_name"),
                                        func.count(User.id).label("no_of_students"),
                                        func.avg(Enrollment.grade).label("average_mark")).join(Teaches).join(
                Course).join(Enrollment).filter(Enrollment.grade > 0).join(User, User.id == Enrollment.student_id,
                                                                           aliased=True).group_by(Course.id)
        ...
        if teacher_disciplines_statistic_form.validate_on_submit():
            ...
            statistics_table = db.session.query(Course.name.label("courses_name"),
                ...
                .join(Enrollment, Enrollment.course_id == Course.id).join(User,
                                                                          User.id == Enrollment.student_id).filter(
                Teaches.teacher_id == teacher_id).all()
        ...
        return render_template("statistics/generate_statistic.html", statistic_id=statistic_id,
                               form=teacher_disciplines_statistic_form)
    else:
